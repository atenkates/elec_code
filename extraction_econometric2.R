...
MainDir<-"G:/DC_Users/atenkat/IrvFile/Elec/PMLs New files"
...
#Precio.marginal.local....MWh.
  temp0<-temp0 %>% mutate(lag.value=lag(Precio.marginal.local....MWh., n = 1, default = NA,order_by = Hora))
  temp0<-temp0 %>% mutate( Price.diff=(log(Precio.marginal.local....MWh./lag.value)) )
...
dfReg<-dfReg %>% group_by(Zona.de.Carga, fecha.iso) %>%
 summarise( mean.pri.diff=sum(Price.diff, na.rm=T), sd.price=sd(Precio.marginal.local....MWh.),
            mean.pri=mean(Precio.marginal.local....MWh.), week.year=first(week),
...
qplot(1:458,dfReg[key,3], main=paste("Zona:",TEST[key[1],1]),xlab = "Weeks",
 ...
 plot(1:length(key),t(TEST[key,3]), type="o" ,
      xlim=c(length(key)-60,length(key)), ylim=c(-.1,0.5),
      main=paste("Zona:",TEST[key[1],1]))

 plot(1:length(key),t(TEST[key,3]), type="o" ,
          main=paste("Zona:",TEST[key[1],1]))
 abline(v=146, col="red")
